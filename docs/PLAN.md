...
1. **Confirm the Redis client shape.** Read `api/routes/health.py:44` to see exactly how the current codebase constructs a sync `redis.Redis` from `settings.redis_url`. Reuse that construction so behavior matches (timeouts, connection args).
